...
import { NormalizedFullOptions, defaultOptions } from "../options";
import { TYPES, QUOTE, BLANK, IDENTIFIER } from "../util/character";
import { Trie } from "../util/operator-trie";
import { Expression } from "../render/expression";
import { PicxTagToken } from "../tokens/picx-tag-token";
...
export class Tokenizer {
  ...
  constructor(public input: string, private trie: Trie = defaultOptions.operatorsTrie, public file: string = "") {
    this.N = input.length;
  }
  ...
  *readExpressionTokens(): IterableIterator<Token> {
  ...
  readPicxTagTokens(options: NormalizedFullOptions = defaultOptions): PicxTagToken[] {
    const tokens: PicxTagToken[] = [];
    while (this.p < this.N) {
      const token = this.readPicxTagToken(options);
      if (token.name) tokens.push(token);
    }
    return tokens;
  }

  readPicxTagToken(options: NormalizedFullOptions): PicxTagToken {
    const { file, input } = this;
    const begin = this.p;
    let end = this.N;
    if (this.readToDelimiter("\n") !== -1) end = this.p;
    return new PicxTagToken(input, begin, end, options, file);
  }
